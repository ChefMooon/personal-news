...
export function useYouTubeViewConfig(instanceId: string): {
  ...
  const [config, setConfigState] = useState<YouTubeViewConfig>(
  ...
  useEffect(() => {
    ...
    window.api
      ...
      .then((value) => {
        setConfigState({
          ...DEFAULT_YOUTUBE_VIEW_CONFIG,
          ...(value as Partial<YouTubeViewConfig>),
        });
      })
      .catch((err) => {
        toast.error(
          err instanceof Error
            ? err.message
            : "Failed to load YouTube view settings.",
        );
      })
      .finally(() => {
        setLoading(false);
    ...
    window.api
      ...
      .catch((err) => {
        toast.error(
          err instanceof Error
            ? err.message
            : "Failed to save YouTube view settings.",
        );
      });
  };
