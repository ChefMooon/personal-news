...
function isSportsViewMode(
  value: string,
): value is SportsViewConfig["viewMode"] {
...
export function normalizeSportsViewConfig(
  config: Partial<SportsViewConfig>,
): SportsViewConfig {
  ...
  const viewMode =
    rawViewMode && isSportsViewMode(rawViewMode)
      ? rawViewMode
...
export function useSportsViewConfig(instanceId: string): {
  ...
  const [config, setConfigState] = useState<SportsViewConfig>(
    DEFAULT_SPORTS_VIEW_CONFIG,
  );
      ...
      .catch((err) => {
        toast.error(
          err instanceof Error
            ? err.message
            : "Failed to load Sports widget settings.",
        );
      });
  }, [storageKey]);
    ...
    window.api
      ...
      .catch((err) => {
        toast.error(
          err instanceof Error
            ? err.message
            : "Failed to save Sports widget settings.",
        );
      });
  };
