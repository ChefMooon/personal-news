...
function applyTheme(theme: ThemeInfo): void {
  ...
  if (resolvedId === "system") {
    resolvedId = window.matchMedia("(prefers-color-scheme: dark)").matches
      ? "dark"
      : "light";
  }
...
export function ThemeProvider({
  ...
  const [theme, setThemeState] = useState<ThemeInfo>({
    id: "system",
    tokens: null,
  });
  ...
  useEffect(() => {
    Promise.all([
      window.api.invoke(IPC.SETTINGS_GET_THEME),
      window.api.invoke(IPC.THEMES_LIST),
    ])
      .then(([t, rows]) => {
  ...
  return (
    <ThemeContext.Provider
      value={{ theme, customThemes, refreshThemes, setTheme }}
    >
      {children}
    </ThemeContext.Provider>
  );
