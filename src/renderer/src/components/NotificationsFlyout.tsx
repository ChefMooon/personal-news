  ...
  useLayoutEffect(() => {
    const updatePosition = (): void => {
      ...
      const maxLeft = Math.max(
        VIEWPORT_MARGIN_PX,
        window.innerWidth - FLYOUT_WIDTH_PX - VIEWPORT_MARGIN_PX,
      );
      const maxTop = Math.max(
        VIEWPORT_MARGIN_PX,
        window.innerHeight - FLYOUT_HEIGHT_PX - VIEWPORT_MARGIN_PX,
      );

      setPosition({
        top: Math.min(rect.bottom + FLYOUT_GAP_PX, maxTop),
        left: Math.min(
          Math.max(rect.right - FLYOUT_WIDTH_PX, VIEWPORT_MARGIN_PX),
          maxLeft,
        ),
      });
    };
            ...
            {notifications.map((notification) => (
              ...
              >
                ...
                <Button
                  ...
                  size="sm"
                  onClick={(event) =>
                    void handleMarkReadClick(event, notification.id)
                  }
