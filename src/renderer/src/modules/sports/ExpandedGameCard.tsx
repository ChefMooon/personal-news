...
function mergeDetailWithEvent(
  detail: SportEventDetail,
  event: SportEvent,
): SportEventDetail {
  return {
    ...detail,
      ...
      <div className="mt-4 rounded-xl border border-dashed px-3 py-3 text-sm text-muted-foreground">
        <div className="flex items-start justify-between gap-3">
          ...
          <Button
            ...
            aria-label="Close radio station results"
          >
            <X className="h-4 w-4" />
          </Button>
        </div>
      ...
      <div className="mb-3 flex items-center justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Only stations that exposed a playable stream during search are
            shown.
          </p>
        </div>
      ...
      <div className="space-y-2">
        {stations.map((station) => (
          <button
            ...
            type="button"
            onClick={() =>
              void playStation(station, formatRadioGameLabel(event))
            }
...
export function ExpandedGameCard({
  ...
  const loadDetails = React.useCallback(async () => {
    ...
    try {
      const result = (await window.api.invoke(IPC.SPORTS_GET_EVENT_DETAILS, {
        eventId: event.eventId,
      })) as SportEventDetail | null;
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground">
              {resolvedDetail.sport}
            </span>
          </div>
          <div className="mt-3 flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2">
              <TeamAvatar
                name={resolvedDetail.awayTeam}
                src={resolvedDetail.awayTeamBadgeUrl}
                className="h-10 w-10 rounded-full"
                fallbackClassName="text-sm"
              />
              <span className="text-xl font-semibold">
                {resolvedDetail.awayTeam}
              </span>
            </div>
            ...
            <div className="flex items-center gap-2">
              <TeamAvatar
                name={resolvedDetail.homeTeam}
                src={resolvedDetail.homeTeamBadgeUrl}
                className="h-10 w-10 rounded-full"
                fallbackClassName="text-sm"
              />
              <span className="text-xl font-semibold">
                {resolvedDetail.homeTeam}
              </span>
            </div>
          </div>
          <p className="mt-1 text-sm text-muted-foreground">
            {formatEventDateTime(resolvedDetail)}
            {resolvedDetail.venue ? ` · ${resolvedDetail.venue}` : ""}
          </p>
        </div>
            ...
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-2 text-3xl font-semibold tabular-nums">
                  {resolvedDetail.awayScore ?? "—"} -{" "}
                  {resolvedDetail.homeScore ?? "—"}
                </p>
              </div>
              <div className="text-right">
                <p className="text-sm font-medium">
                  {resolvedDetail.status ?? "Status unavailable"}
                </p>
                {detail?.progress ? (
            ...
            <dl className="mt-3 space-y-3 text-sm">
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <dd className="text-right font-medium">
                  {resolvedDetail.venue ?? "TBD"}
                </dd>
              </div>
              <div className="flex items-center justify-between gap-3">
                <dt className="text-muted-foreground">Status</dt>
                <dd className="text-right font-medium">
                  {resolvedDetail.status ?? "Unknown"}
                </dd>
              </div>
            </dl>
          </div>
