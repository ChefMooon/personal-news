...
export function isEventOnLocalDate(
  event: SportEvent,
  value = new Date(),
): boolean {
...
export function getClosestEventToNow(
  events: SportEvent[],
  now = Date.now(),
): SportEvent | null {
  if (events.length === 0) {
...
export function getClosestScheduledEventToNow(
  events: SportEvent[],
  now = Date.now(),
): SportEvent | null {
  const scheduledEvents = events.filter(
    (event) => getGamePhase(event) === "scheduled",
  );
  return getClosestEventToNow(
    scheduledEvents.length > 0 ? scheduledEvents : events,
    now,
  );
}

export function sortSportsPageEvents(
  left: SportEvent,
  right: SportEvent,
): number {
  ...
  return `${left.awayTeam}${left.homeTeam}`.localeCompare(
    `${right.awayTeam}${right.homeTeam}`,
  );
}
...
export function formatEventTime(event: SportEvent): string {
  ...
  return new Date(
    `${event.eventDate}T${event.eventTime}:00Z`,
  ).toLocaleTimeString([], {
    hour: "numeric",
    minute: "2-digit",
  });
...
export function getOutcomeForTeam(
  event: SportEvent,
  teamId: string,
): TeamOutcome | null {
  const isHome = event.homeTeamId === teamId;
  const teamScore = Number.parseInt(
    isHome ? (event.homeScore ?? "") : (event.awayScore ?? ""),
    10,
  );
  const opponentScore = Number.parseInt(
    isHome ? (event.awayScore ?? "") : (event.homeScore ?? ""),
    10,
  );
...
export function getStandingFormTokens(form: string | null): TeamOutcome[] {
  ...
  return form
    ...
    .filter(
      (token): token is TeamOutcome =>
        token === "W" || token === "L" || token === "D",
    );
}
