...
function formatMetricValue(sport: string, value: number): string {
  ...
  return `${(Math.max(0, Math.round(value)) / 1000).toFixed(3)}`.replace(
    /^0/,
    "",
  );
}
          ...
          >
            ...
            <div className="flex gap-1">
              {[0, 1, 2, 3, 4].map((pill) => (
                <div
                  key={pill}
                  className="h-4 w-4 animate-pulse rounded-full bg-muted/50"
                />
              ))}
            </div>
          </div>
...
export function StandingsTable({
  ...
  const trackedTeamIds = React.useMemo(
    () => new Set(trackedTeams.map((team) => team.teamId)),
    [trackedTeams],
  );
  const trackedNames = React.useMemo(
    () => new Set(trackedTeams.map((team) => team.name.toLowerCase())),
    [trackedTeams],
  );
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            League standings fetched live from upstream sports APIs.
          </p>
              ...
              <table className="min-w-full text-xs">
                <thead className="text-left text-[10px] uppercase tracking-[0.16em] text-muted-foreground">
                  <tr>
                    ...
                    <th className="pb-2 pr-2">
                      {getRecordExtraLabel(league.sport)}
                    </th>
                    <th className="pb-2 pr-2">
                      {getMetricLabel(league.sport)}
                    </th>
                ...
                <tbody>
                  {rows.map((row) => {
                    const tracked =
                      trackedTeamIds.has(row.teamId) ||
                      trackedNames.has(row.teamName.toLowerCase());
                    const form = getStandingFormTokens(row.form);

                    return (
                      <tr
                        key={row.teamId}
                        className={cn("border-t", tracked && "bg-primary/5")}
                      >
                        ...
                        <td className="py-2 pr-2 font-medium tabular-nums">
                          {formatMetricValue(league.sport, row.points)}
                        </td>
