...
export function TodayGamesStrip({
  ...
  const sortedEvents = useMemo(
    () => [...events].sort(sortSportsPageEvents),
    [events],
  );
  ...
  const liveCount = useMemo(
    () => sortedEvents.filter((event) => isLiveStatus(event.status)).length,
    [sortedEvents],
  );
  const selectedEvent = useMemo(
    () =>
      sortedEvents.find((event) => event.eventId === selectedEventId) ??
      sortedEvents[0] ??
      null,
        ...
        >
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              <Badge
                variant="outline"
                className="h-5 px-1.5 text-[10px] uppercase tracking-[0.14em]"
              >
                {selectedEvent.sport}
              </Badge>
            ...
            <div className="mt-2 space-y-1.5">
              <div className="flex items-center gap-2">
                <TeamChip
                  ...
                  badgeUrl={selectedEvent.awayTeamBadgeUrl}
                />
              ...
              <div className="flex items-center gap-2">
                <TeamChip
                  ...
                  badgeUrl={selectedEvent.homeTeamBadgeUrl}
                />
                <p className="truncate text-sm font-semibold">
            ...
            <p className="mt-1 truncate text-xs text-muted-foreground">
              {getGamePhase(selectedEvent) === "scheduled"
        ...
        <div className="min-h-0 overflow-hidden">
          {sortedEvents.length === 0 ? (
            <div className="rounded-2xl border border-dashed px-4 py-5 text-sm text-muted-foreground">
              No games are scheduled today for the currently enabled sports
              leagues.
            </div>
          ) : (
            <ScrollArea
              ref={scrollAreaRef}
              className="rounded-[24px] border border-border/70 bg-background/75 pb-4 shadow-inner"
            >
              ...
              <ScrollBar
                ...
                className="bottom-2 left-3 right-3 h-3.5 p-[2px]"
              />
            </ScrollArea>
          )}
        </div>
