        ...
        {fallbackEvents.length > 0 ? (
          <div className="space-y-2">
            ...
            {fallbackEvents.slice(0, 5).map((game) => {
              ...
              const showStartTime =
                showLiveStartTime && isLive && !!game.eventTime;
                  ...
                  header={
                    <>
                      ...
                      <div className="shrink-0 text-right">
                        <Badge
                          variant="secondary"
                          className={getGamePhaseBadgeClasses(game)}
                        >
                          {getGamePhaseLabel(game)}
                        </Badge>
                        <p className="mt-1 text-xs text-muted-foreground">
                          {showTime ? formatEventTime(game) : game.eventDate}
                        </p>
                      </div>
                    </>
                  }
            ...
            <div className="flex items-center gap-2">
              <h3 className="text-sm font-semibold">
                {getLeagueLabel({
                  ...
                  leagueId:
                    league?.leagueId ?? leagueEvents[0]?.leagueId ?? leagueKey,
                  leaguesById,
              ...
              {showSportLabels ? (
                <Badge variant="outline">
                  {getSportLabel(league?.sport ?? leagueEvents[0]?.sport ?? "")}
                </Badge>
              ) : null}
              {league?.country ? (
              ...
              {leagueEvents.map((game) => {
                ...
                return (
                  <GameCard
                    ...
                    header={
                      <>
                        ...
                        <div className="shrink-0 text-right">
                          <Badge
                            variant="secondary"
                            className={getTodayBadgeClasses(game)}
                          >
                            {getGamePhase(game) === "scheduled"
                              ? "Today"
                              : getGamePhaseLabel(game)}
                    ...
                    footer={
                      getGamePhase(game) === "scheduled" && showTime
                        ? `Date: ${game.eventDate}`
                        : undefined
                    }
                  />
