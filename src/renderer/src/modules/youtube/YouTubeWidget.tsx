...
import {
  useYouTubeViewConfig,
  DEFAULT_YOUTUBE_VIEW_CONFIG,
} from "../../hooks/useYouTubeViewConfig";
...
function computeDisplayedChannels(
  ...
  if (config.channelOrder.length > 0) {
    ...
    channels = [...channels].sort((a, b) => {
      const ai = orderMap.has(a.channel_id)
        ? orderMap.get(a.channel_id)!
        : Number.MAX_SAFE_INTEGER;
      const bi = orderMap.has(b.channel_id)
        ? orderMap.get(b.channel_id)!
        : Number.MAX_SAFE_INTEGER;
    ...
    <Card className="flex h-full min-h-0 flex-col overflow-hidden">
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      Reset all YouTube widget settings to their defaults? This
                      cannot be undone.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
      ...
      >
        <div className={isEditing ? "youtube-card-edit" : undefined}>
          <div className={isEditing ? "youtube-card-edit__preview" : undefined}>
            {channelList}
          </div>
          {isEditing && (
