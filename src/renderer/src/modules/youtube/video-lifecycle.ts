...
export function isActiveLivestream(video: YtVideo): boolean {
  return (
    video.broadcast_status === "live" || video.broadcast_status === "upcoming"
  );
}
...
export function getYouTubeLifecycleSortTime(video: YtVideo): number {
  ...
  if (video.broadcast_status === "live") {
    return (
      video.actual_start_time ?? video.scheduled_start ?? video.published_at
    );
  }
  if (video.is_livestream === 1) {
    return (
      video.actual_end_time ?? video.actual_start_time ?? video.published_at
    );
  }
