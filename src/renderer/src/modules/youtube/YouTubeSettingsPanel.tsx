  ...
  return (
    ...
    >
      ...
      <button
        ...
        aria-label={
          isPinned ? `Unpin ${channel.name}` : `Pin ${channel.name} to top`
        }
              ...
              >
                <div className="flex items-center justify-between px-3 py-2 bg-muted/30">
                  ...
                  <Switch
                    ...
                    aria-label={
                      hasOverride
                        ? `Disable overrides for ${ch.name}`
                        : `Enable overrides for ${ch.name}`
                    }
...
export function YouTubeSettingsPanel({
    ...
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Failed to load YouTube channels panel state.",
      );
      setChannelsExpanded(true);
    } finally {
      setChannelsExpandedLoaded(true);
  ...
  useEffect(() => {
    ...
    try {
      window.localStorage.setItem(
        channelsExpandedStorageKey,
        String(channelsExpanded),
      );
    } catch (error) {
      toast.error(
        error instanceof Error
          ? error.message
          : "Failed to save YouTube channels panel state.",
      );
    }
  }, [
    channelsExpanded,
    channelsExpandedLoaded,
    instanceId,
    channelsExpandedStorageKey,
  ]);
  ...
  const visibleSelectedChannelIds = useMemo(
    () =>
      draft.selectedChannelIds.filter((id) =>
        channels.some((c) => c.channel_id === id),
      ),
  ...
  const selectedChannelCount =
    draft.channelMode === "all"
      ? channels.length
      : visibleSelectedChannelIds.length;
  ...
  function handleChannelDragEnd(event: DragEndEvent): void {
    ...
    const oldIndex = orderedChannels.findIndex(
      (c) => c.channel_id === active.id,
    );
    const newIndex = orderedChannels.findIndex((c) => c.channel_id === over.id);
    const newOrder = arrayMove(orderedChannels, oldIndex, newIndex).map(
      (c) => c.channel_id,
    );
              ...
              <div id="youtube-settings-channels">
                <div className="flex gap-2 mb-3">
                  <Button
                    size="sm"
                    variant={
                      draft.channelMode === "all" ? "default" : "outline"
                    }
                    onClick={() =>
                      applyUpdate({ ...draft, channelMode: "all" })
                    }
                  >
                    All Channels
                  </Button>
                  <Button
                    size="sm"
                    variant={
                      draft.channelMode === "selected" ? "default" : "outline"
                    }
                    onClick={() =>
                      applyUpdate({ ...draft, channelMode: "selected" })
                    }
                  >
                    Selected Only
                  </Button>
                    ...
                    <button
                      ...
                      onClick={() =>
                        applyUpdate({
                          ...
                          selectedChannelIds: channels.map((c) => c.channel_id),
                        })
                      }
                    >
                      Select all
                      ...
                      >
                        {orderedChannels.map((ch) => (
                          <SortableChannelRow
                            ...
                            isSelected={draft.selectedChannelIds.includes(
                              ch.channel_id,
                            )}
                            isPinned={draft.pinnedChannelIds.includes(
                              ch.channel_id,
                            )}
                            onToggleSelect={() =>
                              toggleChannelSelected(ch.channel_id)
                            }
                            onTogglePin={() =>
                              toggleChannelPinned(ch.channel_id)
                            }
                          />
                        ))}
                      </SortableContext>
            ...
            <div className="space-y-0.5">
              <SettingRow label="Hide watched videos">
                <Switch
                  ...
                  onCheckedChange={(value) =>
                    applyUpdate({ ...draft, hideWatched: value })
                  }
                />
              </SettingRow>
          ...
          <div>
            ...
            >
              <Switch
                ...
                onCheckedChange={(v) =>
                  applyUpdate({ ...draft, showUpcomingPanel: v })
                }
              />
            </SettingRow>
          </div>
            ...
            <div className="space-y-0.5">
              <SettingRow label="Max videos per channel">
                ...
                >
                  <SelectTrigger
                    ...
                    aria-label="Max videos per channel"
                  >
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <SettingRow label="Sort videos by">
                <Select
                  ...
                  onValueChange={(v) =>
                    applyUpdate({
                      ...draft,
                      videoSortDirection: v as "newest" | "oldest",
                    })
                  }
                >
                  <SelectTrigger
              ...
              <SettingRow label="Card density">
                <Select
                  ...
                  onValueChange={(v) =>
                    applyUpdate({
                      ...draft,
                      cardDensity: v as "compact" | "detailed",
                    })
                  }
                >
                  <SelectTrigger
              ...
              <SettingRow label="Show channel headers">
                <Switch
                  ...
                  onCheckedChange={(v) =>
                    applyUpdate({ ...draft, showChannelHeaders: v })
                  }
                />
              </SettingRow>
