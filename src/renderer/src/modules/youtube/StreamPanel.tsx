...
export function StreamPanel({ streams }: StreamPanelProps): React.ReactElement {
  ...
  const streamIdentitySignature = upcomingStreams.map((stream) => stream.video_id).join('|')
  const totalPages = Math.max(1, Math.ceil(upcomingStreams.length / STREAMS_PER_PAGE))
