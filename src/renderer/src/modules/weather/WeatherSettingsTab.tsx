...
export function WeatherSettingsTab(): React.ReactElement {
      ...
      .catch((err) => {
        toast.error(
          err instanceof Error
            ? err.message
            : "Failed to load weather sync status.",
        );
      });
  };
  ...
  const persist = async (
    ...
    const saved = await saveSettings({
      ...
      ...overrides,
      pollIntervalMinutes:
        Number.parseInt(intervalValue, 10) || settings.pollIntervalMinutes,
      thresholds: {
        ...settings.thresholds,
        ...
        freezeTempC:
          Number.parseFloat(freezeValue) || settings.thresholds.freezeTempC,
        heatTempC:
          Number.parseFloat(heatValue) || settings.thresholds.heatTempC,
      },
    });
    setSaving(false);
    if (saved) {
  ...
  const refreshNow = async (): Promise<void> => {
    ...
    try {
      ...
      if (!result.ok) {
        ...
        toast.success(
          `Refreshed ${result.refreshedCount} weather location${result.refreshedCount === 1 ? "" : "s"}.`,
        );
        reloadStatus();
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-6 max-w-3xl pb-8">
        ...
        <div className="grid gap-3 md:grid-cols-2 max-w-2xl">
          ...
          <div className="rounded-md border px-3 py-3 space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Saved locations: {status?.locationCount ?? locations.length}
            </p>
            <p className="text-xs text-muted-foreground">
              Last updated: {lastUpdatedLabel}
            </p>
            <p className="text-xs text-muted-foreground">
              Stale locations: {status?.staleLocationCount ?? 0}
            </p>
            <Button
              variant="outline"
              size="sm"
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mb-3">
          Locations are shared across all Weather widgets. Each widget can use
          the default location or override it.
        </p>
              ...
              {searchResults.map((result) => (
                ...
                >
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => void addLocation(result)}
                  >
                    Add
                  </Button>
                </div>
                  ...
                  >
                    <div className="min-w-0">
                      <p className="text-sm font-medium flex items-center gap-2">
                        ...
                        {isDefault && (
                          <span className="text-xs text-sky-600">Default</span>
                        )}
                      </p>
                      <p className="text-xs text-muted-foreground truncate">
                        {location.timezone}
                    ...
                    <div className="flex items-center gap-2">
                      {!isDefault && (
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            void persist({ defaultLocationId: location.id })
                          }
                        >
                          Set Default
                        </Button>
        ...
        <div className="grid gap-3 md:grid-cols-2 max-w-2xl">
          <div className="rounded-md border px-3 py-3">
            ...
            <Select
              ...
              onValueChange={(value) =>
                void persist({
                  temperatureUnit: value as typeof settings.temperatureUnit,
                })
              }
            >
          ...
          <div className="rounded-md border px-3 py-3">
            ...
            <Select
              ...
              onValueChange={(value) =>
                void persist({
                  windSpeedUnit: value as typeof settings.windSpeedUnit,
                })
              }
            >
              <SelectTrigger className="h-8">
          ...
          <div className="rounded-md border px-3 py-3">
            ...
            <Select
              ...
              onValueChange={(value) =>
                void persist({
                  precipitationUnit: value as typeof settings.precipitationUnit,
                })
              }
            >
              <SelectTrigger className="h-8">
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mb-3">
          Threshold-based alerts are used as a fallback when official weather
        ...
        <div className="space-y-3 max-w-2xl">
          <div className="flex items-center justify-between rounded-md border px-3 py-2">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Controls inline alert chips and messages inside the Weather
                widget.
              </p>
            </div>
            <Switch
              ...
              onCheckedChange={(checked) =>
                void persist({ showAlertsInWidgets: checked })
              }
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <div className="rounded-md border px-3 py-3">
              ...
              <div className="flex gap-2 items-center">
                <Input
                  value={rainValue}
                  onChange={(event) => setRainValue(event.target.value)}
                  inputMode="decimal"
                  className="w-28"
                />
            ...
            <div className="rounded-md border px-3 py-3">
              ...
              <div className="flex gap-2 items-center">
                <Input
                  value={snowValue}
                  onChange={(event) => setSnowValue(event.target.value)}
                  inputMode="decimal"
                  className="w-28"
                />
            ...
            <div className="rounded-md border px-3 py-3">
              ...
              <div className="flex gap-2 items-center">
                <Input
                  value={windValue}
                  onChange={(event) => setWindValue(event.target.value)}
                  inputMode="decimal"
                  className="w-28"
                />
            ...
            <div className="rounded-md border px-3 py-3">
              ...
              <div className="flex gap-2 items-center">
                <Input
                  value={freezeValue}
                  onChange={(event) => setFreezeValue(event.target.value)}
                  inputMode="decimal"
                  className="w-28"
                />
            ...
            <div className="rounded-md border px-3 py-3">
              ...
              <div className="flex gap-2 items-center">
                <Input
                  value={heatValue}
                  onChange={(event) => setHeatValue(event.target.value)}
                  inputMode="decimal"
                  className="w-28"
                />
                <span className="text-xs text-muted-foreground">°C</span>
