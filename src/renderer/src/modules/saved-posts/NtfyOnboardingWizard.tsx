...
export function NtfyOnboardingWizard({
  ...
  const [serverUrl, setServerUrl] = useState(
    initialServerUrl || "https://ntfy.sh",
  );
  ...
  const handleDone = async (): Promise<void> => {
    try {
      ...
      toast.error(
        err instanceof Error ? err.message : "Failed to save ntfy settings.",
      );
    }
  };
    ...
    try {
      ...
      toast.error(
        err instanceof Error
          ? err.message
          : "Failed to reach the ntfy server. Check your topic and server URL.",
      );
    } finally {
      setTesting(false);
    }
        ...
        {step === 1 && (
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              ...
              <span className="font-medium text-foreground">ntfy.sh</span>, a
              free push notification service.
            </p>
            <p className="text-sm text-muted-foreground">
              When you find a post you want to save, share its URL to your ntfy
              topic. Personal News will pick it up automatically and store it
              with full metadata.
            </p>
            <p className="text-sm text-muted-foreground">
              You can optionally add a personal note by putting it on the lines
              after the URL.
            </p>
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Use a random topic name for privacy. Anyone with the name can
                send messages.
              </p>
            </div>
            <div>
            ...
            <div className="rounded-md bg-muted/50 p-3 text-xs text-muted-foreground">
              Note: ntfy.sh retains messages for 24 hours. Open Personal News at
              least once a day to avoid missing posts.
            </div>
            <div className="flex justify-between pt-2">
              ...
              <div className="space-y-2 text-sm text-muted-foreground">
                ...
                <ol className="list-decimal list-inside space-y-1.5 text-xs">
                  ...
                  <li>
                    ...
                    <span className="font-medium">Ask for Input</span> (line 1 =
                    URL, line 2 = notes) or{" "}
                    <span className="font-medium">Get Clipboard</span> (URL
                    only, no notes).
                  </li>
                  <li>
                    Add a{" "}
                  ...
                  <li>
                    Set <span className="font-medium">Get contents of</span> to
                    your ntfy topic URL:
                  ...
                  <li>
                    ...
                    your chosen input (
                    <span className="font-medium">Ask for Input</span> or{" "}
                    <span className="font-medium">Clipboard</span>). Use raw
                    text only (no key required).
                  </li>
                </ol>
                <p className="text-xs mt-2">
                  Tip: when using Ask for Input, enter the Reddit URL on line 1
                  and your note on line 2.
                </p>
              </div>
            ) : (
              <div className="space-y-2 text-sm text-muted-foreground">
                ...
                <ol className="list-decimal list-inside space-y-1.5 text-xs">
                  <li>
                    Install{" "}
                    <span className="font-medium">HTTP Request Shortcuts</span>{" "}
                    from the Play Store.
                  </li>
                  ...
                  <li>
                    Method: POST, Body: plain text with the shared text
                    variable.
                  </li>
                  <li>
                    Enable &quot;Show in Share Menu&quot; so it appears in
                    Android&apos;s share sheet.
                  </li>
                </ol>
                <p className="text-xs mt-2">
                  To add a note: put the Reddit URL on the first line and your
                  note on the following lines.
                </p>
              </div>
            )}
