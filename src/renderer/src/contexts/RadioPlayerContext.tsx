...
export function RadioPlayerProvider({
  ...
  useEffect(() => {
    ...
    const handleWaiting = (): void => {
      setState((current) =>
        current.station ? { ...current, isConnecting: true } : current,
      );
    };
    ...
    const handleError = (): void => {
      ...
      if (!timedOutRef.current) {
        toast.error("Stream unavailable.", {
          description:
            "Stations are community-listed — coverage is not guaranteed.",
        });
      }
  ...
  const searchStations = async (game: SportEvent): Promise<void> => {
    ...
    try {
      const stations = (await window.api.invoke(
      ...
      setSearchError(
        extractIpcErrorMessage(error) ?? "Failed to search radio stations.",
      );
      setState((current) => ({
        ...current,
        stationsLoading: false,
  ...
  const playStation = async (
    ...
    try {
      streamUrl =
        station.playableStreamUrl ??
        ((await window.api.invoke(IPC.SPORTS_RESOLVE_RADIO_STREAM, {
      ...
      toast.error(
        extractIpcErrorMessage(error) ??
          "Unable to resolve a playable radio stream.",
      );
      return;
    }
