...
function SavedPostsContent(): React.ReactElement {
        ...
        .catch((err) => {
          toast.error(
            err instanceof Error
              ? err.message
              : "Failed to load Saved Posts analytics.",
          );
        })
        .finally(() => {
          if (!silent) {
  ...
  const handleSyncNow = async (): Promise<void> => {
    ...
    try {
      ...
      toast.error(
        err instanceof Error
          ? err.message
          : "Failed to sync Saved Posts. Check your ntfy settings.",
      );
    } finally {
      setSyncing(false);
    }
  ...
  const handleDeleteSelected = (): void => {
    setDeleting(true);
    window.api
      .invoke(IPC.REDDIT_DELETE_SAVED_POSTS, {
        post_ids: Array.from(selectedPostIds),
      })
      .then((result) => {
    ...
    <div className="flex flex-col h-full px-6 py-4">
      ...
      <div className="flex items-center gap-3 mb-4">
        <div className="relative flex-1 max-w-xs">
          <label htmlFor="saved-posts-search" className="sr-only">
            Search posts
          </label>
        ...
        <Select value={source ?? "_all"} onValueChange={handleSourceChange}>
          <SelectTrigger
            ...
            {posts.map((post) => (
              <SavedPostItemActions
                ...
                onOpenPost={(currentPost) => {
                  ...
                  const url =
                    currentPost.source === "reddit"
                      ? toRedditPostUrl(currentPost.permalink)
                      : currentPost.url;
                  window.api
                    .invoke(IPC.SHELL_OPEN_EXTERNAL, url)
                    .catch((err) => {
                      toast.error(
                        err instanceof Error
                          ? err.message
                ...
                onSetViewed={(currentPost, viewed) =>
                  setPostViewed(currentPost.post_id, viewed)
                }
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          const url =
                            post.source === "reddit"
                              ? toRedditPostUrl(post.permalink)
                              : post.url;
                          window.api
                            .invoke(IPC.SHELL_OPEN_EXTERNAL, url)
                            .catch((err) => {
