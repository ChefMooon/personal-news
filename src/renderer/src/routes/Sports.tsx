...
import {
  ...
  getClosestScheduledEventToNow,
} from "../modules/sports/page-utils";
import {
  SPORTS_PAGE_SPORT_ORDER_KEY,
  normalizeSportOrder,
} from "../modules/sports/sport-order";
...
import {
  ...
  type SportsDataFetchWarningEvent,
  type TrackedTeam,
...
export default function SportsPage(): React.ReactElement {
  ...
  const [sportOrder, setSportOrder] =
    React.useState<SupportedSport[]>(SUPPORTED_SPORTS);
  ...
  const [teamEventsById, setTeamEventsById] = React.useState<
    Record<string, SportTeamEvents>
  >({});
  const [standingsByLeague, setStandingsByLeague] = React.useState<
    Record<string, SportStandingRow[]>
  >({});
  const [loading, setLoading] = React.useState(true);
  const [refreshing, setRefreshing] = React.useState(false);
  const [updating, setUpdating] = React.useState(false);
  const [todayGamesCollapsed, setTodayGamesCollapsed] = React.useState(false);
  const [selectedEvent, setSelectedEvent] = React.useState<SportEvent | null>(
    null,
  );
      ...
      .catch((error) => {
        toast.error(
          error instanceof Error
            ? error.message
            : "Failed to load sports page order.",
        );
        setOrderReady(true);
      });
  }, []);
    ...
    window.api
      ...
      .catch((error) => {
        toast.error(
          error instanceof Error
            ? error.message
            : "Failed to save sports page order.",
        );
      });
  }, [orderReady, sportOrder]);

  const loadTeamEvents = React.useCallback(
  ...
  const loadData = React.useCallback(
    async ({ preserveContent = false }: { preserveContent?: boolean } = {}) => {
      ...
      try {
        ...
        const teams = (allTeams as TrackedTeam[]).filter(
          (team) => team.enabled,
        );
        ...
        toast.error(
          error instanceof Error
            ? error.message
            : "Failed to load Sports page data.",
        );
      } finally {
        if (loadId === latestLoadIdRef.current) {
          setLoading(false);
  ...
  const enabledLeaguesBySport = React.useMemo(() => {
    ...
    for (const sport of sportOrder) {
      map.set(
        sport,
        leagues.filter((league) => league.sport === sport && league.enabled),
      );
    }
    return map;
  }, [leagues, sportOrder]);
  ...
  const visibleTodayEvents = React.useMemo(
    () => filterEventsForLocalDate(todayEvents),
    [todayEvents],
  );
  const autoScrollEvent = React.useMemo(
    () => getClosestScheduledEventToNow(visibleTodayEvents),
    [visibleTodayEvents],
  );
  ...
  React.useEffect(() => {
    setSelectedEvent((current) => {
      ...
      return (
        visibleTodayEvents.find((event) => event.eventId === current.eventId) ??
        null
      );
    });
  }, [visibleTodayEvents]);

  const getStandingForTeam = React.useCallback(
    (team: TrackedTeam): SportStandingRow | null => {
      const rows = standingsByLeague[team.leagueId] ?? [];
      return (
        rows.find(
          (row) =>
            row.teamId === team.teamId ||
            row.teamName.toLowerCase() === team.name.toLowerCase(),
        ) ?? null
      );
    },
    [standingsByLeague],
  ...
  const handleStandingsRowsLoaded = React.useCallback(
    (leagueId: string, rows: SportStandingRow[]): void => {
      ...
      <div className="flex min-h-full items-center justify-center px-6 py-10">
        <div className="max-w-lg rounded-2xl border bg-card p-6 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Enable the Sports feature from Settings to access the full sports
            page and live radio player.
          </p>
        </div>
      </div>
    );
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <h1 className="mt-2 text-2xl font-semibold">
                Season context and today&apos;s action
              </h1>
              <p className="mt-1 max-w-3xl text-sm text-muted-foreground">
                Track live games, league tables, team momentum, and best-effort
                radio coverage from one dedicated sports view.
              </p>
            </div>
          ...
          <div className="space-y-10">
            {sportOrder.map((sport) => {
              ...
              if (
                ...
                !visibleTodayEvents.some((event) => event.sport === sport)
              ) {
                return null;
              }
                  ...
                  <div className="flex flex-wrap items-end justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-muted-foreground">
                        {sportTeams.length} tracked teams ·{" "}
                        {sportLeagues.length} enabled leagues
                      </p>
                    </div>
                  </div>
