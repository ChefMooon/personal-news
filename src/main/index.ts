...
function validatePersistedBounds(
  raw: string | null,
...
function ensureVisibleBounds(
  ...
  const intersectsAnyDisplay = displays.some(
    (display) => getIntersectionArea(bounds, display.workArea) > 0,
  );
  ...
  return {
    x:
      primaryWorkArea.x +
      Math.max(0, Math.floor((primaryWorkArea.width - bounds.width) / 2)),
    y:
      primaryWorkArea.y +
      Math.max(0, Math.floor((primaryWorkArea.height - bounds.height) / 2)),
...
function getInitialWindowOptions(): {
  ...
  const restoreWindowBounds = getBooleanSetting(
    RESTORE_WINDOW_BOUNDS_SETTING_KEY,
    true,
  );
  ...
  const persisted = validatePersistedBounds(
...
function getTrayIconPath(): string {
  const preferredName = nativeTheme.shouldUseDarkColors
    ? "tray-light"
    : "tray-dark";
  const preferredExtensions =
    process.platform === "win32" ? ["png", "svg"] : ["png", "svg"];

  for (const extension of preferredExtensions) {
    const preferredPath = getResourcePath(
      "resources",
      "tray",
  ...
  for (const extension of fallbackExtensions) {
    const fallbackPath = getResourcePath(
      "resources",
      "tray",
      `tray-dark.${extension}`,
    );
