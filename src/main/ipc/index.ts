...
function logYouTubeResolveDebug(
  _message: string,
...
function buildTemporaryChannelThumbnail(
  channelId: string,
  channelName: string,
): string {
...
function emitYoutubeVideoWatchedChanged(
  event: YoutubeVideoWatchedChangedEvent,
): void {
  for (const win of BrowserWindow.getAllWindows()) {
...
function buildSavedPostsQueryParts(options?: {
  ...
  if (options?.no_tags_only) {
    conditions.push(
      "(sp.tags IS NULL OR sp.tags = '[]' OR json_array_length(sp.tags) = 0)",
    );
  }
...
const LEGACY_INSTANCE_CONFIG_KEYS: Partial<
  Record<(typeof DASHBOARD_CONFIG_KEY_PREFIXES)[number], string>
> = {
...
function getLegacyWidgetLayout(): WidgetLayout {
  const orderRaw =
    getSetting("widget_order") ?? '["youtube","reddit_digest","saved_posts"]';
  const visibilityRaw =
  ...
  return {
    ...
    widget_instances: JSON.parse(instancesRaw) as Record<
      string,
      WidgetInstance
    >,
  };
...
function readInstanceConfigForClone(
  prefix: (typeof DASHBOARD_CONFIG_KEY_PREFIXES)[number],
  instanceId: string,
): string | null {
...
function deleteWidgetInstanceConfigs(instanceIds: string[]): void {
  const uniqueIds = Array.from(
    new Set(instanceIds.filter((value) => value.trim().length > 0)),
  );
...
function cloneWidgetInstanceConfigs(
  cloneOperations: DashboardViewsMutation["cloneInstanceConfigs"],
): void {
  ...
  for (const operation of cloneOperations) {
    ...
    for (const prefix of DASHBOARD_CONFIG_KEY_PREFIXES) {
      const value = readInstanceConfigForClone(
        prefix,
...
async function validateRedditDigestSubreddit(
  subreddit: string,
): Promise<IpcMutationResult> {
  ...
  if (!/^[a-z0-9_]+$/i.test(normalized)) {
    return {
      ok: false,
      error: "Subreddits may contain letters, numbers, and underscores only.",
    };
  }

  const params = new URLSearchParams({ t: "week", limit: "1" });
  const response = await fetch(
    `https://www.reddit.com/r/${normalized}/top.json?${params.toString()}`,
    {
  ...
  if (!response.ok || payload.error) {
    throw new Error(
      payload.error?.message ??
        "Failed to fetch channel details from YouTube API.",
    );
  }
...
async function fetchChannelFromRss(
  channelId: string,
): Promise<YtChannel | null> {
...
function extractChannelIdFromHtml(html: string): {
  ...
  if (canonicalMatch?.[1]) {
    return {
      ...
      matchedBy: "canonical-link-channel",
    };
  }
...
async function resolveChannelWithoutApiKey(
  ...
  if (parsed.channelId) {
    ...
    if (!found) {
      throw new Error(
        "Could not load channel feed. Verify the channel ID and try again.",
      );
    }
    return found;
  }
  ...
  if (!trimmed) {
    throw new Error(
      "Enter a channel ID, @handle, or full YouTube channel URL.",
    );
  }
  ...
  if (!pageResolved.channelId) {
    throw new Error(
      "Could not resolve a channel from that URL. Use a channel ID, @handle, or channel URL.",
    );
  }
  ...
  return {
    ...fromRss,
    name:
      fromRss.name === fromRss.channel_id
        ? pageResolved.title || fromRss.name
        : fromRss.name,
...
async function resolveChannelFromInput(input: string): Promise<YtChannel> {
  ...
  if (!candidateUrl) {
    logYouTubeResolveDebug("resolveFromInput:noCandidateUrl", {
      input,
      parsedQuery: parsed.query,
    });
    throw new Error(
  ...
  if (input.type === "on_app_start") {
    return {
      scheduleJson: JSON.stringify({
        type: "on_app_start" satisfies ScheduleDef["type"],
      }),
      isManual: false,
      error: null,
    };
  ...
  if (input.type === "interval") {
    ...
    if (!Number.isFinite(minutes) || minutes < 1 || minutes > 1440) {
      return {
        scheduleJson: null,
        isManual: false,
        error: "Interval minutes must be between 1 and 1440.",
      };
    }
    return {
      scheduleJson: JSON.stringify({
  ...
  if (input.type === "weekly") {
    ...
    if (days.length === 0) {
      return {
        scheduleJson: null,
        isManual: false,
        error: "Select at least one day of week.",
      };
    }
    return {
      scheduleJson: JSON.stringify({
...
function normalizeYouTubeVideosFilterOptions(
  ...
  Pick<YouTubeVideosFilterOptions, "channelId" | "search" | "hideWatched"> & {
    mediaTypes: MediaType[];
  } {
  const validMediaTypes = new Set<MediaType>([
    "video",
    "short",
    "upcoming_stream",
    "live",
  ...
  const normalizedMediaTypes = Array.isArray(options?.mediaTypes)
    ? options.mediaTypes.filter((mediaType): mediaType is MediaType =>
        validMediaTypes.has(mediaType),
      )
    : [];
...
export function registerIpcHandlers(): void {
  ...
  ipcMain.handle(IPC.YOUTUBE_GET_CHANNELS, (): YtChannel[] => {
    const db = getDb();
    return db
      .prepare("SELECT * FROM yt_channels ORDER BY sort_order")
      .all() as YtChannel[];
  });

  // youtube:getVideos
  ipcMain.handle(
    IPC.YOUTUBE_GET_VIDEOS,
    (_event, channelId: string): YtVideo[] => {
      const db = getDb();
      return db
        .prepare(
          `SELECT *
  ...
  ipcMain.handle(
    ...
    (
      _event,
      options?: YouTubeVideosFilterOptions,
    ): YouTubeVideosFilterResult => {
      const db = getDb();
      ...
      if (normalized.mediaTypes.length > 0) {
        ...
        whereClauses.push(
          `${YOUTUBE_EFFECTIVE_MEDIA_TYPE_SQL} IN (${placeholders})`,
        );
  ...
  ipcMain.handle(
    ...
    async (_event, input: string): Promise<IpcMutationResult> => {
      ...
      try {
        ...
        const temporaryThumbnail = buildTemporaryChannelThumbnail(
          channel.channel_id,
          channel.name,
        );
        ...
        if (existing) {
          ...
          const row = db
            .prepare(
              "SELECT COALESCE(MAX(sort_order), -1) AS max_sort FROM yt_channels",
            )
            .get() as {
            max_sort: number;
          };
  ...
  ipcMain.handle(
    ...
    (_event, channelId: string): IpcMutationResult => {
      const db = getDb();
      const result = db
        .prepare("DELETE FROM yt_channels WHERE channel_id = ?")
        .run(channelId);
      if (result.changes === 0) {
        return { ok: false, error: "Channel not found." };
      }
    ...
    try {
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to poll YouTube RSS feeds.",
      };
    }
  });
  ...
  ipcMain.handle(
    ...
    (_event, channelId: string): IpcMutationResult => {
      ...
      db.prepare(
        "UPDATE yt_videos SET watched_at = ? WHERE channel_id = ? AND watched_at IS NULL",
      ).run(watchedAt, channelId);
      emitYoutubeUpdated();
      return { ok: true, error: null };
    },
  );
  ...
  ipcMain.handle(
    ...
    (_event, options?: RedditDigestPostsRequest): DigestPost[] => {
      const db = getDb();
      const { whereClause, params } = buildDigestWhereClause(options);
      return db
        .prepare(
          `SELECT * FROM reddit_digest_posts ${whereClause} ORDER BY week_start_date DESC, fetched_at DESC`,
        )
  ...
  ipcMain.handle(
    ...
    (
      ...
      const result = db
        .prepare(
          "UPDATE reddit_digest_posts SET viewed_at = ? WHERE post_id = ? AND week_start_date = ?",
        )
  ...
  ipcMain.handle(
    ...
    (
      ...
      const { whereClause, params } = buildDigestWhereClause(options);
      const stateClause = options.viewed
        ? "viewed_at IS NULL"
        : "viewed_at IS NOT NULL";
      const mergeWhere = whereClause
        ? `${whereClause} AND ${stateClause}`
        : `WHERE ${stateClause}`;
      const result = db
  ...
  ipcMain.handle(
    ...
    (
      _event,
      options?: PruneDigestOptions,
    ): { ok: boolean; error: string | null; deletedCount: number } => {
      const db = getDb();
      ...
      if (options?.delete_week) {
        if (!/^\d{4}-\d{2}-\d{2}$/.test(options.delete_week)) {
          return {
            ok: false,
            error: "Invalid week_start_date.",
            deletedCount: 0,
          };
        }
        const result = db
          .prepare("DELETE FROM reddit_digest_posts WHERE week_start_date = ?")
          .run(options.delete_week);
        emitRedditUpdated();
        return { ok: true, error: null, deletedCount: result.changes };
      }
      ...
      if (typeof options?.keep_weeks === "number") {
        ...
        if (!Number.isFinite(keepWeeks) || keepWeeks < 1) {
          return {
            ok: false,
            error: "keep_weeks must be at least 1.",
            deletedCount: 0,
          };
        }
        ...
        const weeksToKeep = weeks
          .slice(0, keepWeeks)
          .map((row) => row.week_start_date);
        const placeholders = weeksToKeep.map(() => "?").join(", ");
        const result = db
          .prepare(
            `DELETE FROM reddit_digest_posts WHERE week_start_date NOT IN (${placeholders})`,
          )
          .run(...weeksToKeep);
        emitRedditUpdated();
        return { ok: true, error: null, deletedCount: result.changes };
      ...
      return {
        ok: false,
        error: "No prune operation was specified.",
        deletedCount: 0,
      };
    },
  );
  ...
  ipcMain.handle(
    ...
    (
      ...
      const subredditFilter =
        options?.subreddit_filter ??
        (options?.subreddit ? [options.subreddit] : undefined);
      const tagFilter =
        options?.tag_filter ?? (options?.tag ? [options.tag] : undefined);
  ...
  ipcMain.handle(
    ...
    (_event, postId: string, viewed: boolean): IpcMutationResult => {
      ...
      const viewedAt = viewed ? Math.floor(Date.now() / 1000) : null;
      const result = db
        .prepare("UPDATE saved_posts SET viewed_at = ? WHERE post_id = ?")
        .run(viewedAt, postId);
      if (result.changes === 0) {
  ...
  ipcMain.handle(
    ...
    (
      ...
      const db = getDb();
      const viewedAt = options.viewed ? Math.floor(Date.now() / 1000) : null;
      const { fromClause, whereClause, params } =
        buildSavedPostsQueryParts(options);
      const stateClause = options.viewed
        ? "sp.viewed_at IS NULL"
        : "sp.viewed_at IS NOT NULL";
      const mergeWhere = whereClause
        ? `${whereClause} AND ${stateClause}`
        : `WHERE ${stateClause}`;
      const ids = db
      ...
      const update = db.prepare(
        "UPDATE saved_posts SET viewed_at = ? WHERE post_id = ?",
      );
  ...
  ipcMain.handle(
    ...
    (_event, postId: string, note: string | null): IpcMutationResult => {
      ...
      const result = db
        .prepare("UPDATE saved_posts SET note = ? WHERE post_id = ?")
        .run(normalizedNote, postId);
      if (result.changes === 0) {
        return { ok: false, error: "Saved post not found." };
      }
      emitRedditUpdated();
  ...
  ipcMain.handle(IPC.REDDIT_POLL_NTFY, async (): Promise<NtfyPollResult> => {
    ...
    const lastPolledAt =
      parseInt(getSetting("ntfy_last_polled_at") ?? "0", 10) || 0;
    return {
  ...
  ipcMain.handle(IPC.SCRIPTS_GET_ALL, (): ScriptWithLastRun[] => {
    ...
    const rows = db
      ...
      .all() as Array<
      ScriptWithLastRun & { last_success_finished_at: number | null }
    >;
  ...
  ipcMain.handle(
    IPC.SCRIPTS_UPDATE,
    (_event, input: ScriptUpdateInput): IpcMutationResult => {
      ...
      if (schedule.isManual && input.enabled) {
        return {
          ok: false,
          error: "Manual schedule cannot have auto-run enabled.",
        };
      }
  ...
  ipcMain.handle(
    ...
    (
      _event,
      scriptId: number,
      scheduleInput: ScriptScheduleInput,
    ): IpcMutationResult => {
      ...
      const db = getDb();
      const existing = db
        .prepare("SELECT enabled FROM scripts WHERE id = ?")
  ...
  ipcMain.handle(
    ...
    (_event, scriptId: number, enabled: boolean): IpcMutationResult => {
      if (!Number.isInteger(scriptId) || scriptId <= 0) {
        return { ok: false, error: "Invalid script ID." };
      }
      ...
      const db = getDb();
      const existing = db
        .prepare("SELECT schedule FROM scripts WHERE id = ?")
        .get(scriptId) as { schedule: string | null } | undefined;
      ...
      if (!existing.schedule && enabled) {
        return {
          ok: false,
          error: "Cannot enable auto-run when schedule is manual.",
        };
      }
  ...
  ipcMain.handle(
    IPC.SCRIPTS_CANCEL,
    (_event, scriptId: number): { ok: boolean; error: string | null } => {
      if (!Number.isInteger(scriptId) || scriptId <= 0) {
        return { ok: false, error: "Invalid script ID." };
      }
  ...
  ipcMain.handle(
    ...
    (_event, scriptId: number): ScriptRunRecord[] => {
      if (!Number.isInteger(scriptId) || scriptId <= 0) {
        return [];
      }
      const db = getDb();
  ...
  ipcMain.handle(
    IPC.SCRIPTS_GET_NOTIFICATIONS,
    (_event, limit?: number): ScriptNotification[] => {
      const db = getDb();
      const safeLimit = Number.isInteger(limit)
        ? Math.min(Math.max(limit as number, 1), 200)
        : 100;
      return db
        .prepare(
  ...
  ipcMain.handle(
    IPC.SETTINGS_SET_WIDGET_LAYOUT,
    (_event, layout: WidgetLayout): void => {
  ...
  ipcMain.handle(
    IPC.SETTINGS_SET_DASHBOARD_VIEWS,
    (_event, mutation: DashboardViewsMutation): void => {
  ...
  ipcMain.handle(
    IPC.SETTINGS_SET_SIDEBAR_CONFIG,
    (_event, config: SidebarConfig): IpcMutationResult => {
      setSetting(
        SIDEBAR_CONFIG_SETTING,
        JSON.stringify(normalizeSidebarConfig(config)),
      );
      return { ok: true, error: null };
    },
  );
  ...
  ipcMain.handle(IPC.THEMES_LIST, (): ThemeRow[] => {
    ...
    const rows = db
      .prepare(
        "SELECT id, name, tokens, created_at FROM themes ORDER BY created_at ASC",
      )
      .all() as Array<{
      id: string;
      name: string;
  ...
  ipcMain.handle(
    IPC.THEMES_CREATE,
    (_event, name: string, tokensInput: unknown): ThemeRow => {
      ...
      if (!tokens) {
        throw new Error(
          "Theme tokens must be a non-empty map of CSS variables to values.",
        );
      }
      ...
      db.prepare(
        "INSERT INTO themes (id, name, tokens, created_at) VALUES (?, ?, ?, ?)",
      ).run(id, trimmedName, JSON.stringify(tokens), createdAt);
  ...
  ipcMain.handle(
    IPC.THEMES_UPDATE,
    (_event, id: string, name: string, tokensInput: unknown): ThemeRow => {
      ...
      const row = db
        .prepare("SELECT created_at FROM themes WHERE id = ?")
  ...
  ipcMain.handle(
    IPC.THEMES_EXPORT,
    async (_event, id: string): Promise<IpcMutationResult> => {
      if (!id || BUILT_IN_THEME_IDS.has(id)) {
      ...
      const raw = db
        ...
        .get(id) as
        | { id: string; name: string; tokens: string; created_at: number }
        | undefined;
      ...
      try {
        const content = JSON.stringify(
          { name: row.name, tokens: row.tokens },
          null,
          2,
        );
  ...
  ipcMain.handle(IPC.THEMES_IMPORT, async (): Promise<ThemeImportResult> => {
    ...
    if (!tokens) {
      return {
        ok: false,
        error: "Theme file has invalid or missing token definitions.",
      };
    }
    ...
    const existingNames = new Set<string>(
      (
        db.prepare("SELECT name FROM themes").all() as Array<{ name: string }>
      ).map((r) => r.name),
    );
    ...
    db.prepare(
      "INSERT INTO themes (id, name, tokens, created_at) VALUES (?, ?, ?, ?)",
    ).run(id, finalName, JSON.stringify(tokens), createdAt);
  ...
  ipcMain.handle(
    IPC.SETTINGS_GET_YOUTUBE_API_KEY_STATUS,
    (): YouTubeApiKeyStatus => {
      try {
  ...
  ipcMain.handle(
    ...
    async (
      _event,
      requestedSubreddits: unknown,
    ): Promise<IpcMutationResult> => {
      const subreddits = normalizeDigestSubredditList(requestedSubreddits);
      if (subreddits.length === 0) {
        return {
          ok: false,
          error: "Choose at least one valid subreddit to sync.",
        };
      }
      ...
      if (scriptId === null) {
        return {
          ok: false,
          error:
            "The bundled Reddit Digest script is unavailable, so the immediate sync could not start.",
        };
      }
      ...
      if (activeRuns.has(scriptId)) {
        return {
          ok: false,
          error:
            "Reddit Digest is already running. Wait for the current run to finish, then try again.",
        };
      }
      ...
      runScriptById(db, script, {
        ...
        console.error(
          `[IPC] reddit:syncDigestSubreddits error for script ${scriptId}:`,
          err,
        );
      });
    ...
    (_event, minutes: number): IpcMutationResult => {
      if (!Number.isInteger(minutes) || minutes < 1 || minutes > 1440) {
        return {
          ok: false,
          error:
            "RSS poll interval must be a whole number between 1 and 1440 minutes.",
        };
      }
    ...
    (_event, minutes: number): IpcMutationResult => {
      if (!Number.isInteger(minutes) || minutes < 1 || minutes > 1440) {
        return {
          ok: false,
          error:
            "ntfy poll interval must be a whole number between 1 and 1440 minutes.",
        };
      }
  ...
  ipcMain.handle(
    IPC.SETTINGS_GET_YOUTUBE_VIEW_CONFIG,
    (_event, instanceId: string): YouTubeViewConfig => {
  ...
  ipcMain.handle(
    ...
    (
      ...
      setSetting(
        `${YOUTUBE_VIEW_CONFIG_KEY_PREFIX}${instanceId}`,
  ...
  ipcMain.handle(
    IPC.WEATHER_SAVE_LOCATION,
    (_event, location: WeatherSearchResult): WeatherLocation => {
      return saveWeatherLocation(location);
    },
  );

  ipcMain.handle(
    IPC.WEATHER_REMOVE_LOCATION,
    (_event, locationId: string): IpcMutationResult => {
      return deleteWeatherLocation(locationId);
    },
  );

  ipcMain.handle(
    IPC.WEATHER_GET_SNAPSHOT,
    (_event, locationId: string): WeatherSnapshot | null => {
  ...
  ipcMain.handle(
    ...
    async (
      _event,
      locationId?: string,
    ): Promise<IpcMutationResult & { refreshedCount: number }> => {
      try {
  ...
  ipcMain.handle(
    IPC.WEATHER_SET_SETTINGS,
    (_event, settings: WeatherSettings): WeatherSettings => {
  ...
  ipcMain.handle(
    ...
    async (): Promise<IpcMutationResult> => {
  ...
  ipcMain.handle(
    ...
    (_event, key: string, value: string): void => {
      const previousValue =
        key === "script_home_dir" || key === REDDIT_DIGEST_SUBREDDITS_SETTING
          ? getSetting(key)
          : null;
      ...
      if (key === "weather_enabled") {
        ...
        if (value === "true" || value === "1") {
          void triggerWeatherRefresh().catch((error) => {
            console.error(
              "[Weather] Failed to refresh after enabling feature:",
              error,
            );
          });
        }
  ...
  ipcMain.handle(
    IPC.SHELL_OPEN_PATH,
    (_event, folderPath: string): Promise<string> => {
