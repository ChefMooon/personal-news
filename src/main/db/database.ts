...
function runMigrations(database: Database.Database): void {
  ...
  const latestVersion = Math.max(
    ...Object.keys(migrationFiles).map((k) => parseInt(k, 10)),
  );
  ...
  while (nextVersion <= latestVersion) {
    ...
    if (!migrationFile) {
      throw new Error(
        `Missing migration file mapping for schema version ${nextVersion}`,
      );
    }
    ...
    if (app.isPackaged) {
      ...
      migrationPath = join(
        __dirname,
        `../../src/main/db/migrations/${migrationFile}`,
      );
    }
    ...
    appliedAny = true;
    console.log(
      `[DB] Migration ${String(nextVersion).padStart(3, "0")} applied`,
    );
    nextVersion += 1;
  }
  ...
  const compatibilityMigrations: Array<{
  ...
  }> = [
    {
      ...
      shouldApply: (db) =>
        !tableExists(db, "weather_locations") ||
        !tableExists(db, "weather_cache"),
    },
    {
      name: "sports-schema",
      file: "003_sports.sql",
      shouldApply: (db) =>
        !tableExists(db, "sports_leagues") || !tableExists(db, "sports_events"),
    },
    {
