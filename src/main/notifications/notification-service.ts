...
export function getNotificationPreferences(): NotificationPreferences {
  ...
  try {
    ...
    return {
      ...
      scriptManager: {
        ...DEFAULT_PREFS.scriptManager,
        ...parsed.scriptManager,
      },
    };
  } catch {
    return { ...DEFAULT_PREFS };
...
export function notifyYoutubeNewVideos(entries: YoutubeVideoEntry[]): void {
  ...
  if (eligible.length === 1) {
    show(
      "New YouTube Video",
      `${eligible[0].channelName}: ${eligible[0].title}`,
    );
    return;
  }
...
export function notifySavedPostsSync(postsIngested: number): void {
  ...
  const label =
    postsIngested === 1
      ? "1 new saved post"
      : `${postsIngested} new saved posts`;
  show("Saved Posts Synced", `Ingested ${label} from ntfy.sh.`);
}

export function notifyWeatherAlerts(
  locationName: string,
  alerts: WeatherAlert[],
): void {
  if (!canNotify()) return;
  ...
  show(
    ...
    `${locationName}: ${alerts.length} active weather alerts.`,
  );
}
...
export function notifyRedditDigest(
  ...
  const title =
    severity === "info" ? "Reddit Digest Complete" : "Reddit Digest Failed";
  show(title, message);
}
