...
const YOUTUBE_SYNC_MANUAL_CONFIRMATION_SETTING =
  "youtube_sync_manual_confirmation";
...
function extractLinkHref(
  link: RawLink | RawLink[] | undefined,
  preferRel?: string,
): string | null {
  if (!link) return null;
...
function parseRssCandidates(
  ...
  for (const entry of entries) {
    ...
    candidates.push({
      ...
      rssPublishedAt:
        typeof entry.published === "string" ? entry.published : null,
    });
  }
...
function getExistingVideoIds(
  ...
  for (const chunk of idChunks) {
    ...
    const rows = database
      .prepare(
        `SELECT video_id FROM yt_videos WHERE video_id IN (${placeholders})`,
      )
...
function needsChannelThumbnailRefresh(
...
function normalizeChannelThumbnailUrl(
...
async function refreshChannelThumbnailsFromApi(
  ...
  for (const ids of channelIdBatches) {
    ...
    const applyBatchUpdates = database.transaction(
      (batchItems: ApiChannelItem[]) => {
        let changedRows = 0;
...
function parseUnixSeconds(
  rawIsoDate: string | null | undefined,
): number | null {
  if (!rawIsoDate) {
    return null;
  }
...
function getTrackedLivestreamRows(
  database: Database.Database,
): TrackedLivestreamRow[] {
  return database
    .prepare(
...
async function pollAllEnabledChannels(): Promise<void> {
  ...
  const channels = dbRef
    .prepare(
      "SELECT channel_id, name, thumbnail_url, notify_new_videos, notify_live_start FROM yt_channels WHERE enabled = 1 ORDER BY sort_order",
    )
    .all() as Array<{
    channel_id: string;
    name: string;
  ...
  const videosToFetch = Array.from(
    new Set([...newVideoIds, ...trackedLivestreamIds]),
  );
  ...
  if (!apiKey) {
    if (channelThumbCandidates > 0) {
      channelThumbSkippedReason =
        "No YouTube API key configured. Skipped channels.list thumbnail refresh.";
    }
  } else {
    const refreshStats = await refreshChannelThumbnailsFromApi(
      dbRef,
      apiKey,
      channels,
    );
  ...
  if (!apiKey && newVideoIds.length > 0) {
    saveDebugSnapshot({
      ...
      updatedCount: 0,
      error:
        "No YouTube API key configured. Skipped videos.list API enrichment.",
      sampleEntries: [],
      normalizedFeed: {
        channel: {
          id: "multi",
  ...
  const allNewVideoEntries: Array<{
    videoId: string;
    channelId: string;
    title: string;
  }> = [];
  const allLiveStartEntries: Array<{
    videoId: string;
    channelId: string;
    title: string;
  }> = [];
  ...
  for (let batchIndex = 0; batchIndex < batches.length; batchIndex += 1) {
    ...
    const sourceChannelIds = Array.from(
      new Set(
        ids.flatMap((id) =>
          (byVideoId.get(id) ?? []).map(
            (candidate) => candidate.sourceChannelId,
          ),
        ),
      ),
    );
    ...
    if (manualDecision === "skip_remaining") {
      ...
      batchDebug.push({
        ...
        error:
          "Manual confirmation decision requested skipping all remaining batches.",
      });
      unresolvedIds.clear();
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Network error during YouTube API call.";
    ...
    if (!response.ok) {
      ...
      enqueueRetryBatch(dbRef!, {
        ...
        normalizedPreviewPath: null,
        reason:
          response.status === 403 ? "quota_or_forbidden" : "api_http_error",
        lastError: errorMessage,
      });
    ...
    const ingestTx = dbRef.transaction(() => {
      ...
      let batchInserted = 0;
      let batchUpdated = 0;
      const newVideoEntries: Array<{
        videoId: string;
        channelId: string;
        title: string;
      }> = [];
      const liveStartEntries: Array<{
        videoId: string;
        channelId: string;
        title: string;
      }> = [];
      ...
      for (const entry of normalizedBatch) {
        ...
        const nextActualStartUnix =
          actualStartUnix ?? alreadyExists?.actual_start_time ?? null;
        const nextActualEndUnix =
          actualEndUnix ?? alreadyExists?.actual_end_time ?? null;
        ...
        if (alreadyExists) {
          ...
          if (
            alreadyExists.broadcast_status !== "live" &&
            nextBroadcastStatus === "live"
          ) {
            liveStartEntries.push({
              videoId: entry.id,
              channelId,
              title: entry.title,
            });
  ...
  const batchFailures = batchDebug.filter(
    (item) =>
      item.status === "failed" || item.status === "normalization_failed",
  );
  const hasApiFailures = batchFailures.length > 0;
  const hasOnlySkipped =
    batchDebug.length > 0 &&
    batchDebug.every((item) => item.status === "skipped");
  const errorMessageParts: string[] = [];

  if (channelErrors.length > 0) {
    errorMessageParts.push(
      `RSS channel failures: ${channelErrors.map((x) => x.channelId).join(", ")}`,
    );
  }
  if (hasApiFailures) {
  ...
  if (hasOnlySkipped) {
    errorMessageParts.push(
      "All API batches were skipped by manual confirmation mode.",
    );
  }
...
function startPollScheduler(): void {
  ...
  if (pollTask) {
    pollTask.stop();
    const taskWithDestroy = pollTask as ScheduledTask & {
      destroy?: () => void;
    };
