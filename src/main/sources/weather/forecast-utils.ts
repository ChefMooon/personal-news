...
export function mapCurrentConditions(
  ...
  return {
    ...
    surfacePressure: currentNumber(
      currentPayload,
      "surface_pressure",
      fallback.surfacePressure,
    ),
    visibility: currentNumber(
      currentPayload,
      "visibility",
      fallback.visibility,
    ),
...
export function takeUpcomingHourly<T extends { time: number }>(
  points: T[],
  currentTime: number,
  count: number,
): T[] {
  const currentHour = floorUnixSecondsToHour(currentTime);
  const firstUpcomingIndex = points.findIndex(
...
export function splitYesterdayFromDaily<T>(
  points: T[],
  forecastDays: number,
): {
  yesterday: T | null;
