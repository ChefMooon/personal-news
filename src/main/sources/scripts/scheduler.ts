...
type RunFn = (
  ...
  context: ScriptScheduleRunContext,
) => Promise<void>;
...
function countFixedTimeWindows(
  lastRunAt: number,
  now: number,
  hour: number,
  minute: number,
...
function countWeeklyWindows(
  ...
  while (
    Math.floor(nextWindow.getTime() / 1000) <= lastRunAt ||
    !daySet.has(nextWindow.getDay())
  ) {
    nextWindow.setDate(nextWindow.getDate() + 1);
  }
...
function getMonthlyWindowTimestamp(
  year: number,
  month: number,
  day: number,
  hour: number,
...
function countMonthlyWindows(
  ...
  while (nextWindow !== null && nextWindow <= now && count <= 500) {
    ...
    do {
      ...
      nextWindow = getMonthlyWindowTimestamp(
        year,
        month,
        dayOfMonth,
        hour,
...
export function decideStartupAction(
  ...
  if (def.type === "monthly") {
    ...
    const missedRuns = countMonthlyWindows(
      lastRunAt,
      now,
      hour,
      minute,
...
export class ScriptScheduler {
  ...
  initialize(
    ...
    runFn: RunFn,
    startupWarningFn: StartupWarningFn,
  ): void {
    this.runFn = runFn;
  ...
  registerScript(
    script: ScriptWithLastRun,
    options?: { runOnAppStart?: boolean; now?: number },
  ): void {
    ...
    try {
      ...
      console.warn(
        `[Scheduler] Invalid schedule JSON for script ${script.id}: ${script.schedule}`,
      );
      return;
    }
    ...
    if (def.type === "on_app_start") {
      if (options?.runOnAppStart) {
        ...
        this.runFn(script, { trigger: "on_app_start" }).catch((err: Error) => {
          console.error(
            `[Scheduler] on_app_start script ${script.name} failed:`,
            err,
          );
        });
      }
    ...
    if (options?.runOnAppStart) {
      ...
      if (decision.action === "catch_up") {
        ...
        this.runFn(script, { trigger: "catch_up" }).catch((err: Error) => {
          console.error(
            `[Scheduler] catch-up script ${script.name} failed:`,
            err,
          );
        });
    ...
    if (
      def.type === "interval" &&
      options?.runOnAppStart &&
      def.run_on_app_start &&
      !startupHandled
    ) {
      console.log(
        `[Scheduler] Running interval script on app start: ${script.name}`,
      );
      this.runFn(script, { trigger: "on_app_start" }).catch((err: Error) => {
        console.error(
          `[Scheduler] interval start-run script ${script.name} failed:`,
          err,
        );
      });
    }
    ...
    if (!expr) {
      console.warn(
        `[Scheduler] Cannot build cron expression for script ${script.id}`,
      );
      return;
    }
    ...
    if (!cron.validate(expr)) {
      console.warn(
        `[Scheduler] Invalid cron expression "${expr}" for script ${script.id}`,
      );
      return;
    }
    ...
    const task = cron.schedule(
      ...
      () => {
        ...
        this.runFn(script, { trigger: "scheduled" }).catch((err: Error) => {
          console.error(
            `[Scheduler] Scheduled script ${script.name} failed:`,
            err,
          );
        });
      },
