...
function parsePlaylistUrls(body: string, baseUrl: string): string[] {
  ...
  const pushCandidate = (value: string): void => {
    ...
    try {
      const candidate = normalizeRemoteUrl(
...
function getStationPriority(station: RadioStation): number {
  ...
  if (
    station.countryCode?.toUpperCase() === "US" ||
    station.countryCode?.toUpperCase() === "CA"
  ) {
    score += 1;
  }
...
export async function resolveSportsRadioStream(
  url: string,
  depth = 0,
): Promise<string> {
  ...
  const looksLikeIcyStream =
    response.headers.has("icy-name") || response.headers.has("icy-br");
  ...
  if (isLikelyHlsUrl(finalUrl) || isHlsContentType(contentType)) {
    cancelResponseBody(response);
    throw new Error(
      "This station uses HLS (adaptive streaming), which is not supported by this player.",
    );
  }
  ...
  if (
    ...
    ((contentType === "" || contentType === "application/octet-stream") &&
      (looksLikeIcyStream || isLikelyDirectAudioUrl(finalUrl)))
  ) {
    cancelResponseBody(response);
    return finalUrl;
  }
  ...
  if (isLikelyPlaylistUrl(finalUrl) || isPlaylistContentType(contentType)) {
    throw new Error(
      "This station publishes a playlist, but no playable stream could be extracted.",
    );
  }
...
function getSearchTerms(game: SportEvent): string[] {
  return Array.from(
    new Set(
      [
        buildCityKeyword(game.homeTeam),
        buildCityKeyword(game.awayTeam),
        game.homeTeam,
        game.awayTeam,
...
function mapStation(row: RadioBrowserStation): RadioStation | null {
  ...
  const bitrateValue =
    typeof row.bitrate === "number"
      ? row.bitrate
      : Number.parseInt(row.bitrate ?? "", 10);
...
export async function searchSportsRadioStations(
  game: SportEvent,
  ...
  const requests = searches.flatMap((name) =>
    countryCodes.map(async (countryCode) => {
      ...
      const response = await fetch(
        `${RADIO_BROWSER_BASE_URL}?${params.toString()}`,
      );
      if (!response.ok) {
        throw new Error(
          `Radio Browser request failed with HTTP ${response.status}.`,
        );
      }
  ...
  if (payloads.length === 0) {
    throw new Error(
      "Could not reach the radio station directory. Check your internet connection and try again.",
    );
  }
  ...
  for (const payload of payloads) {
    for (const row of payload) {
      ...
      const haystack = normalizeSearchText(
        `${station.name} ${station.tags.join(" ")} ${station.country ?? ""}`,
      );
      if (
        !desiredTerms.some(
