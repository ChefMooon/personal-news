...
const SPORTS_STARTUP_REFRESH_STALE_MINUTES_KEY =
  "sports_startup_refresh_stale_minutes";
...
const DEFAULT_ENABLED_LEAGUE_PATTERNS: Partial<
  Record<SupportedSport, RegExp[]>
> = {
    ...
    () => {
      for (const sport of SUPPORTED_SPORTS) {
        void refreshSportInternal(sport, true).catch((error) => {
          console.error(
            `[Sports] Scheduled refresh failed for ${sport}:`,
            error,
          );
        });
      }
...
async function prefetchLeagueTeamBadges(
  ...
  try {
    ...
    console.warn(
      `[Sports] Failed to prefetch team badges for league "${leagueName}":`,
      error,
    );
  }
}
...
function pickBestLiveRefreshCandidate(
  current: SportEvent,
  candidates: SportEvent[],
): SportEvent | null {
  ...
  const matchCandidates = candidates.filter((item) =>
    isSameMatch(current, item),
  );
  ...
  const preferredEspn = matchCandidates.find((item) =>
    item.eventId.startsWith("espn:"),
  );
...
async function searchTeamBadgeByName(
  ...
  const exactNameMatches = matches.filter(
    (match) => normalizeTeamLookupKey(match.name) === teamKey,
  );
  const preferred =
    (leagueKey
      ? exactNameMatches.find(
          (match) => normalizeLeagueLookupKey(match.leagueName) === leagueKey,
        )
      : null) ??
    exactNameMatches[0] ??
    (leagueKey
  ...
  return preferred
    ? { name: preferred.name, badgeUrl: preferred.badgeUrl }
    : null;
}
...
async function backfillEventTeamBadges(
  ...
  let leagueBadgeCatalogByName: Map<
    string,
    { name: string; badgeUrl: string | null }
  > | null = null;
  const searchedTeamNames = new Map<
    string,
    Promise<{ name: string; badgeUrl: string | null } | null>
  >();

  const getLeagueBadgeCatalogByName = async (): Promise<
    Map<string, { name: string; badgeUrl: string | null }>
  > => {
    ...
    leagueBadgeCatalogByName = new Map(
      teams.map(
        (team) =>
          [
            normalizeTeamLookupKey(team.name),
            { name: team.name, badgeUrl: team.badgeUrl },
          ] as const,
      ),
    );
    return leagueBadgeCatalogByName;
  ...
  for (const [teamId, fallbackName] of teamCandidates) {
    ...
    const fallbackMatch =
      !details?.badgeUrl && leagueName
        ? (await getLeagueBadgeCatalogByName()).get(
            normalizeTeamLookupKey(fallbackName),
          )
        : null;
    ...
    if (!details?.badgeUrl && !fallbackMatch?.badgeUrl) {
      ...
      const pending =
        searchedTeamNames.get(searchKey) ??
        searchTeamBadgeByName(fallbackName, sport, leagueName);
    ...
    const badgeUrl =
      details?.badgeUrl ??
      fallbackMatch?.badgeUrl ??
      searchMatch?.badgeUrl ??
      null;
    const resolvedName =
      details?.name ?? fallbackMatch?.name ?? searchMatch?.name ?? fallbackName;
...
function isBadgeRefreshStale(
  fetchedAt: number | null | undefined,
  nowSeconds: number,
): boolean {
  return (
    fetchedAt == null ||
    nowSeconds - fetchedAt >= BADGE_REFRESH_INTERVAL_SECONDS
  );
}

async function syncLeagueCatalog(
  sport: string,
  forceRefresh = false,
): Promise<SportLeague[]> {
  const db = ensureDb();
...
async function refreshEnabledLeagueBadges(
  sport: string,
  force: boolean,
): Promise<void> {
  const db = ensureDb();
  ...
  const shouldRefreshAny =
    force ||
    existingEnabledLeagues.some((league) => {
  ...
  for (const league of enabledLeagues) {
    ...
    const cachedLeagueEvents = readTodayEvents(db, league.sport, today).filter(
      (event) => event.leagueId === league.leagueId,
    );
    if (cachedLeagueEvents.length > 0) {
      await backfillEventTeamBadges(
        db,
        cachedLeagueEvents,
        league.sport,
        league.name,
...
async function refreshSportsBadgesInternal(
  sport: string,
  force: boolean,
): Promise<void> {
...
async function ensureLeagueCatalogLoaded(
  sport: string,
): Promise<SportLeague[]> {
...
function isEventDataStale(event: SportEvent): boolean {
  ...
  const scoresUnchanged =
    state.lastScore.home === event.homeScore &&
    state.lastScore.away === event.awayScore;
...
function defaultLeagueEnabled(sport: string, league: SportLeague): boolean {
  ...
  return (
    DEFAULT_ENABLED_LEAGUE_PATTERNS[sport]?.some((pattern) =>
      pattern.test(league.name),
    ) ?? false
  );
}

async function refreshTrackedTeam(
  team: TrackedTeam,
  fetchedDate: string,
): Promise<void> {
  const db = ensureDb();
...
async function doLiveRefresh(sport: string): Promise<void> {
  ...
  const liveEvents = readTodayEvents(db, sport, fetchDate).filter((event) =>
    isLiveSportEvent(event),
  );
  const refreshedEvents: SportEvent[] = [];
  const leagueNamesById = new Map(
    listEnabledLeagues(db, sport).map(
      (league) => [league.leagueId, league.name] as const,
    ),
  );

  for (const event of liveEvents) {
    const isEspnBackedEvent = event.eventId.startsWith("espn:");
    const refreshedEvent = isEspnBackedEvent
      ? null
      : await fetchEventById(event.eventId);
    ...
    if (shouldAttemptFallback) {
      try {
        ...
        if (
          state &&
          state.espnRetries === ESPN_FAILURE_NOTIFICATION_THRESHOLD
        ) {
          emitSportsFetchWarning({
            sport,
            message:
              "Live sports data temporarily unavailable, using cached score",
            severity: "warning",
            timestamp: Date.now(),
          });
        }
...
async function doRefreshSport(sport: string, force: boolean): Promise<void> {
  ...
  for (const league of enabledLeagues) {
    ...
    for (const eventDate of fetchDates) {
      const eventsForDate = await fetchLeagueEventsForDateWithFallback(
        eventDate,
        league.leagueId,
        league.sport,
        league.name,
...
async function refreshSportInternal(
  sport: string,
  force: boolean,
): Promise<void> {
...
export function updateSportsSettings(
  settings: Partial<SportsSettings>,
): SportsSettings {
  if (settings.pollIntervalMinutes !== undefined) {
    const clamped = normalizeSportsPollIntervalMinutes(
  ...
  if (settings.startupRefreshStaleMinutes !== undefined) {
    const clamped = normalizeSportsStartupRefreshStaleMinutes(
      settings.startupRefreshStaleMinutes,
    );
      ...
      .catch((error) => {
        emitSportsUpdated({
          ...
          error:
            error instanceof Error
              ? error.message
              : "Failed to fetch team sports data.",
        });
      });
...
export function removeSportsTeam(teamId: string): IpcMutationResult {
  ...
  emitSportsUpdated({
    sport: team?.sport ?? DEFAULT_SPORT,
    ok: true,
    error: null,
  });
...
export function setSportsTeamEnabled(
  teamId: string,
  enabled: boolean,
): IpcMutationResult {
  const team = getTrackedTeam(ensureDb(), teamId);
...
export function setSportsTeamOrder(orderedIds: string[]): IpcMutationResult {
  ...
  const updatedSport = orderedIds[0]
    ? (getTrackedTeam(ensureDb(), orderedIds[0])?.sport ?? DEFAULT_SPORT)
    : DEFAULT_SPORT;
...
export async function addSportsLeague(
  leagueId: string,
  sport: string,
): Promise<SportLeague> {
  const db = ensureDb();
  const existing = (await ensureLeagueCatalogLoaded(sport)).find(
  ...
  const updated = listLeagues(db, sport).find(
    (league) => league.leagueId === leagueId,
  );
  if (!updated) {
...
export function removeSportsLeague(leagueId: string): IpcMutationResult {
  ...
  emitSportsUpdated({
    sport: league?.sport ?? DEFAULT_SPORT,
    ok: true,
    error: null,
  });
  return { ok: true, error: null };
...
export async function refreshSportsBadges(
  sport: string,
  force = true,
): Promise<void> {
