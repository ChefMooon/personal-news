...
const FINAL_STATUS_PATTERN =
  /(finished|final|completed|game over|ended|after penalties|after extra time|full time|full-time|match finished|final score|result|\bft\b|\baet\b)/i;
const LIVE_STATUS_PATTERN =
  /(live|in progress|half time|halftime|break|period|quarter|inning|set\s*\d|overtime|extra time|top\s*\d|bottom\s*\d|ot|shootout|penalties|intermission|1st inning|2nd inning|3rd inning|4th inning|5th inning|6th inning|7th inning|8th inning|9th inning|\b(?:1st|2nd|3rd|4th)\b|\d{1,3}(?:\+\d{1,2})?['’])/i;
const SCHEDULED_STATUS_PATTERN =
  /(scheduled|not started|not begun|preview|to be played|fixture|time tba|tba|tbd|upcoming)/i;
const NON_LIVE_STATUS_PATTERN =
  /(postponed|cancelled|canceled|abandoned|suspended|delayed|no contest|n\/c)/i;
