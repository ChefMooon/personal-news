...
export const SUPPORTED_SPORTS = SPORTS_OPTIONS.map(
  (option) => option.id,
) as SupportedSport[];
...
export function normalizeSportsPollIntervalMinutes(value: number): number {
  return Math.max(
    ...
    Math.min(MAX_SPORTS_POLL_INTERVAL_MINUTES, Math.round(value)),
  );
}

export function normalizeSportsStartupRefreshStaleMinutes(
  value: number,
): number {
  return Math.max(
...
export function getSportLabel(sport: string): string {
  return (
    SPORTS_WIDGET_OPTIONS.find((option) => option.id === sport)?.label ?? sport
  );
}
