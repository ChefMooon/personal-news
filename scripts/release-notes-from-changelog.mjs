...
function printUsage() {
  console.log('Usage: node scripts/release-notes-from-changelog.mjs <version> [channel] [--dry-run]')
  console.log('')
...
function normalizeChannel(channel) {
  ...
  if (normalized !== 'release' && normalized !== 'alpha' && normalized !== 'beta') {
    throw new Error(`Invalid channel: ${channel}. Expected release, alpha, or beta.`)
  }
  return normalized
}
...
function extractChangelogNotes(changelogContent, version) {
  ...
  const sectionHeaderPattern = new RegExp(`^## \\[${version.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\] - `)
  ...
  const sectionLines = lines.slice(startIndex + 1, endIndex >= 0 ? endIndex : lines.length)
...
function runGhReleaseEdit(tagVersion, notesFilePath) {
  const result = spawnSync('gh', ['release', 'edit', tagVersion, '--notes-file', notesFilePath], {
    stdio: 'inherit',
  ...
  if (result.status !== 0) {
    throw new Error(`gh release edit failed for ${tagVersion} with exit code ${String(result.status)}.`)
  }
}
...
function main() {
  ...
  if (!assertSemver(version)) {
    throw new Error(`Invalid version: ${version}. Expected semantic version format x.y.z.`)
  }
  ...
  const tempDir = mkdtempSync(path.join(tmpdir(), 'personal-news-release-notes-'))
