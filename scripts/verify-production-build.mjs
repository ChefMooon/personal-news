...
const SOURCE_MIGRATIONS_DIR = path.join(
  ROOT,
  "src",
  "main",
  "db",
...
function getExpectedSchemaVersion() {
  ...
  if (versions.length === 0) {
    throw new Error(
      "No migration files were found under src/main/db/migrations.",
    );
  }
...
function run(command, args) {
  ...
  if (typeof result.status !== "number" || result.status !== 0) {
    throw new Error(
      `Command failed (${result.status ?? "unknown"}): ${fullCommand}`,
    );
  }
}
...
function findInstallerPath() {
  ...
  if (!setupFile) {
    throw new Error(
      "Expected Windows setup executable was not found under dist/.",
    );
  }
...
function verifySmokeReport() {
  ...
  if (report.packaged !== true) {
    throw new Error(
      "Smoke report indicates app was not running in packaged mode.",
    );
  }
  ...
  if (!existsSync(report.dbPath)) {
    throw new Error(
      `Smoke test database file does not exist: ${report.dbPath}`,
    );
  }
...
function runPackagedSmokeTest() {
  ...
  try {
    ...
    const result = spawnSync(
      EXE_PATH,
      ["--smoke-test", `--smoke-output=${SMOKE_OUTPUT_PATH}`],
      {
    ...
    if (typeof result.status !== "number" || result.status !== 0) {
      if (report?.error) {
        throw new Error(
          `Packaged smoke test failed (${result.status ?? "unknown"}): ${report.error}`,
        );
      }
    ...
    assertPathExists(
      SMOKE_OUTPUT_PATH,
      "Packaged smoke test did not write a smoke report.",
    );
    const reportRaw = readFileSync(SMOKE_OUTPUT_PATH, "utf-8");
    ...
    if (Number.isNaN(reportTimestamp) || reportTimestamp < startedAt) {
      throw new Error(
        "Packaged smoke test report is stale or has an invalid timestamp.",
      );
    }
...
function main() {
  if (process.platform !== "win32") {
    throw new Error(
      "verify-production-build is currently implemented for Windows only.",
    );
  }
