...
The release workflow is triggered only when you push a git tag that matches `v*.*.*` (example: `v1.2.0`).

## One-Time Context

- GitHub Action: `.github/workflows/release-tag.yml`
- Trigger: `push` on tags like `v1.2.0`
- Build/publish command used by CI: `npm run build:win -- --publish always`

## Release Steps (v1.2.0)
...
2. Confirm `CHANGELOG.md` includes the `1.2.0` section and final notes.

3. Update app version in `package.json` and `package-lock.json` to `1.2.0`.
...
8. Open GitHub Actions and watch the `Release (Windows)` workflow for the `v1.2.0` tag run.
...
2. Confirm installer asset exists (example name: `Personal News-1.2.0-Setup-x64.exe`).
...
- Tag/version mismatch (`package.json` is `1.2.0` but tag is different).
...
## If You Need to Re-Run v1.2.0 Tag
